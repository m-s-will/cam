...
{
public:
  FuzzyEQ(double maxDiff) : MaxDiff(maxDiff) 
  {
  }
...
{
public:
  FuzzyLT(double maxDiff) : MaxDiff(maxDiff)
  {
  }
...
int addAttribute(vtkSmartPointer<vtkUnstructuredGrid> grid, const char* name,
...
public:
  Internals()
  {
    ...
    this->CubeCoordinatesCache[std::pair<int, int>(16,4)] = std::vector<double>(v, v+49);
  }
  ~Internals()
  {
  }
  ...
  std::map<std::pair<int, int>, std::vector<double> > CubeCoordinatesCache;
  //////////////////////////////////////////////////////////////////////
};
...
Grid<gridType>::Grid() : 
  ...
  MpiRank(-1),
  ChunkCapacity(0), NCells2d(0), NLon(0),
  NLat(0), NLev(0), Lev(NULL), Ne(0), Np(0), Radius(0),
  LonStep(0), LatStep(0), Impl(new Internals())
{
}
...
vtkSmartPointer<vtkUnstructuredGrid> Grid<gridType>::CreateGrid2d()
{
  ...
  this->RankArrayIndex2d = 
    addAttribute<vtkIntArray>(grid2d, "Rank", this->NCells2d, 1);
  this->CoordArrayIndex2d = addAttribute<vtkDoubleArray>(
    grid2d, "Coord", this->NCells2d, 2);
  this->PSArrayIndex = 
    addAttribute<vtkDoubleArray>(grid2d, "PS", this->NCells2d, 1);
  ...
  for (int i = 0; i < indexArray.size(); ++i)
    ...
    for (int p = 0; p < 4; ++p)
      ...
      if (it == impl->PointId2d.end())
        {
        realId = impl->PointId2d[pointId] = 
          this->Grid2d->GetPoints()->InsertNextPoint(cellPoints[p][0],
                                                     cellPoints[p][1], 
                                                     cellPoints[p][2]);
        // ostr.str("");
    ...
    assert(impl->CellId2d.find(cellId) == impl->CellId2d.end());
    ...
    for (int level = 0; level < this->NLev; ++level)
      ...
      realCellId = this->Grid3d->InsertNextCell(VTK_HEXAHEDRON, 8, realPointIds);
      int cellId = this->GetCellId(index, level);
      impl->CellId3d[cellId] = realCellId;
      ostr << "Create 3D cell id: " << cellId << " real id: " << realCellId
           << std::endl;
      }
    // std::cerr << ostr.str();
    }
...
template<GridType gridType>
void Grid<gridType>::SetAttributeValue(int chunkSize,
  ...
  for (int c = 0; c < chunkSize; ++c)
    ...
    for (int i = 0; i < indexArray.size(); ++i)
      ...
      for (int level = 0; level < this->NLev; ++level)
        {
        int cellId = this->GetCellId(index, level);
        typename Internals::MapType::iterator cellIt = 
          impl->CellId3d.find(cellId);
        ...
        realCellId = cellIt->second;
        ostr << "Set 3D cell value cell id: " << cellId << " real id: " << realCellId
             << std::endl;
...
void Grid<gridType>::GetCellPoints(const int index[3], double cell[4][3]) const
{
  ...
  if (gridType == CUBE_SPHERE)
    ...
    cellIndex[0][0] = cellIndex[1][0] = (index[0] > 0) ? 
      index[0] - 0.5: index[0];
    cellIndex[2][0] = cellIndex[3][0] = (index[0] < this->NLon - 1) ? 
      index[0] + 0.5: index[0];
    cellIndex[0][1] = cellIndex[3][1] = (index[1] > 0) ?
      index[1] - 0.5: index[1];
    cellIndex[1][1] = cellIndex[2][1] = (index[1] < this->NLon - 1) ? 
      index[1] + 0.5: index[1];
    cellIndex[0][2] = cellIndex[1][2] = cellIndex[2][2] = cellIndex[3][2] = index[2];
    // std::ostringstream ostr;
    // ostr << "cell points - index space: index: " << index[0] << ", " << index[1] << ", " << index[2]
    ...
    for (int i = 0; i < 4; ++i)
      ...
      // ostr.str("");
      // ostr << "cell points - cube space: index: " << index[0] << ", " << index[1] << ", " << index[2]
      ...
      // ostr.str("");
      // ostr << "cell points - spherical: index: " << index[0] << ", " << index[1] << ", " << index[2]
...
void Grid<gridType>::GetCellPoints(const int index[3], int level, double cell[8][3]) const
{
  ...
  if (gridType == CUBE_SPHERE)
    ...
    cellIndex[0][2] = cellIndex[1][2] = cellIndex[2][2] = cellIndex[3][2] = 
      cellIndex[4][2] = cellIndex[5][2] = cellIndex[6][2] = cellIndex[7][2] = 
      index[2];
    // std::ostringstream ostr;
    // ostr << "cell points 3D - index space: index: " << index[0] << ", " << index[1] << ", " << index[2]
    //      << " cell0: " << cellIndex[0][0] << ", " << cellIndex[0][1] << ", " << cellIndex[0][2]
    //      << " cell1: " << cellIndex[1][0] << ", " << cellIndex[1][1] << ", " << cellIndex[1][2]
    //      << " cell2: " << cellIndex[2][0] << ", " << cellIndex[2][1] << ", " << cellIndex[2][2]
    //      << " cell3: " << cellIndex[3][0] << ", " << cellIndex[3][1] << ", " << cellIndex[3][2]
    //      << std::endl;
    // std::cerr << ostr.str();
    double levels[8] = {levMinus, levMinus, levMinus, levMinus,
                        levPlus, levPlus, levPlus, levPlus};
    for (int i = 0; i < 8; ++i)
      {
      // cube space
      this->IndexToCube(impl->CubeCoordinates, cellIndex[i], cell[i]);
      // ostr.str("");
      // ostr << "cell points 3D - cube space: index: " << index[0] << ", " << index[1] << ", " << index[2]
      //      << " cell"  << i << ": "<< cell[i][0] << ", " << cell[i][1] << ", " << cell[i][2]
      //      << std::endl;
      // std::cerr << ostr.str();
      ...
      this->CubeToSpherical(cell[i], &lonRad, &latRad);
      // ostr.str("");
      // ostr << "cell points 3D - spherical: index: " << index[0] << ", " << index[1] << ", " << index[2]
      //      << " cell"  << i << ": "<< vtkMath::DegreesFromRadians(lonRad) 
      //      << ", " << vtkMath::DegreesFromRadians(latRad)
      //      << std::endl;
      // std::cerr << ostr.str();
...
template<GridType gridType>
void Grid<gridType>::GetPointIds(
  const int index[3], int pointIds[4]) const
{
  ...
  if (gridType == CUBE_SPHERE)
    ...
    for (int p = 0; p < 4; ++p)
      ...
      pointIds[p] = tempPointId[0] + 
        tempPointId[1] * (this->NLon + 1) + 
...
template<GridType gridType>
void Grid<gridType>::GetPointIds(
  const int index[3], int level, int pointIds[8]) const
{
  int s = index[0], t = index[1];
  int temp[8][3] = {{s,   t,   level},
                    {s+1, t,   level},
                    {s+1, t+1, level},
                    {s,   t+1, level},
                    {s,   t,   level+1},
...
void Grid<gridType>::MinFaceIndex(int side, int index[3]) const
{
  ...
  double indexD[3] = {index[0], index[1], index[2]};
  double sideD = side;
  ...
  int mfi = this->CubeToIndex(
    sideD, cubeCoordinates, v, allIndexes);
  ...
  double lonRad, double latRad, std::vector<std::vector<int> >& index) const
{
  ...
  if (gridType == CUBE_SPHERE)
    ...
    int oneIndex[3] =
    {
      round (lonDeg / this->LonStep),          // interval [0, 360]
      round ((90 + latDeg) / this->LatStep),   // interval [-90, 90]
      0                                        // not used
    };
    index.resize(1);
    index[0].resize(3);
...
template<GridType gridType>
bool Grid<gridType>::SetToCoprocessor(
  vtkCPDataDescription* coprocessorData, const char* name, vtkSmartPointer<vtkUnstructuredGrid> grid)
{
  ...
  int ne, int np, int lonSize, double* lonRad, int latSize, double* latRad)
{
  ...
  std::map<std::pair<int,int>, std::vector<double> >::iterator it = 
    impl->CubeCoordinatesCache.find(std::pair<int,int>(ne, np));
  if (it != impl->CubeCoordinatesCache.end())
    {
    std::cerr << "Found cached grid coordinates\n";
    impl->CubeCoordinates = it->second;
    }
  else
    {
    ...
    if (this->MpiRank == 0)
      ...
      std::ostringstream ostr;
      for (int i = 0; i < impl->CubeCoordinates.size(); ++i)
        {
      
        ostr << std::setprecision(std::numeric_limits<double>::digits10) 
             << impl->CubeCoordinates[i] << std::endl;
        }
      of << ostr.str();
      }
    }
...
template<GridType gridType>
int Grid<gridType>::CubeToIndex (
  double side, const std::vector<double>& cubeCoordinates,
  double v[3], std::vector<std::vector<int> >& index) const
{
  ...
  for (int i = 0; i < NFACES; ++i)
    ...
    if (eq(v[valIndex[i].index[0]], valIndex[i].value))
      {
      std::vector<double>::const_iterator it1 = lower_bound(cubeCoordinates.begin(),
                                                 cubeCoordinates.end(),
                                                 v[valIndex[i].index[1]],
                                                 FuzzyLT1E_10());
      std::vector<double>::const_iterator it2 = lower_bound(cubeCoordinates.begin(),
                                                 cubeCoordinates.end(),
                                                 v[valIndex[i].index[2]],
                                                 FuzzyLT1E_10());
      assert (it1 != cubeCoordinates.end() &&
              it2 != cubeCoordinates.end());
      int oneIndex[3] = {
        it1 - cubeCoordinates.begin(),
        it2 - cubeCoordinates.begin(),
        valIndex[i].face};
  ...
  double index[3], double v[3]) const
{
  ...
  for (int i = 0; i < 2; ++i)
    ...
    if (eq(floorIndex, cubeCoordinates.size() - 1))
      ...
      v[position[face][i]] = cubeCoordinates[floorIndex] * (1 - t) +
        cubeCoordinates[floorIndex + 1] * t;
      }
    }
}

template<GridType gridType>
void Grid<gridType>::CubeToSpherical (
  double v[3], double* lonRad, double* latRad) const
{
